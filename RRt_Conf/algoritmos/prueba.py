...
GOAL_SAMPLING_RATE = 0.20     # prob. de muestrear el goal
...
STAGNATION_ITERS = 300        # iter sin mejora para forzar TURN
STAGNATION_DELTA = 1.0        # mejora minima de d_target
...
ENTER_TURN        = 200       # fallos para entrar en TURN
EXIT_STREAK       = 300       # exitos para salir de TURN
MODE_FREEZE       = 500     # enfriamiento entre cambios de modo
...
def plan_rrt_star(smap: SewerMap, start: State, goal: State, vis_img, waypoints=None, corridor_poly=None):
    ...
    for it in range(MAX_ITERS):
        ...
        if (it+1) % DRAW_EVERY == 0 and segs and SHOW_RRT_TREE:
            ...
            tmp = base_img.copy()
            # start/goal sin detalles para capturas limpias
        ...
        if best_state is not None and (
            ...
            # Si quieres ver soporte/CM del goal solo cuando SHOW_STATES=True
            if SHOW_STATES:
            ...
            print(f"[METRICS] elapsed={elapsed:.3f}s | iters={it+1} | length={length_m:.2f}m ({length_px:.1f}px) | "
                  f"prop={n_propagations} invalid={n_invalid} ({fail_pct:.1f}%)")
    ...
    elapsed = t1 - t0
    fail_pct = 100.0 * (n_invalid / max(1, n_propagations))
    print(f"[METRICS] elapsed={elapsed:.3f}s | iters={MAX_ITERS} | NO_SOLUTION | "
          f"prop={n_propagations} invalid={n_invalid} ({fail_pct:.1f}%)")
    ...
    while True:
        ...
        if k == 32:  # SPACE
            ...
            if SHOW_ASTAR_ONLY:
                ...
                cv2.waitKey(0)  # se queda fijo para screenshot
                continue

            plan_img, sol_points, nodes, iters = plan_rrt_star(
